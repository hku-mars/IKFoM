...
typedef MTK::S2<double, 981, 100, 1> S2; 
...
/*
// only use manifold as a data holder for process noise, not necessary
...
Eigen::Matrix<double, 12, 12> process_noise_cov()
{
	...
	cov.template block<3, 3>(0, 0) = Eigen::Matrix3d::Identity() * 0.0001;
	cov.template block<3, 3>(3, 3) = Eigen::Matrix3d::Identity() * 0.0001;
	cov.template block<3, 3>(6, 6) = Eigen::Matrix3d::Identity() * 0.001;
	cov.template block<3, 3>(9, 9) = Eigen::Matrix3d::Identity() * 0.001;
	return cov;
}
