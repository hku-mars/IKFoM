...
void ImuProcess::IMU_Initial(const MeasureGroup &meas, ekf &state_inout, int &N)
{
  ...
  esekfom::esekf<state, 12, measurement, input>::cov init_P =esekfom::esekf<state, 12, input>::cov::Identity() * INIT_COV;
...
void ImuProcess::UndistortPcl(const MeasureGroup &meas, ekf &state_inout, PointCloudXYZI &pcl_out)
{
  ...
  for (auto it_imu = v_imu.begin(); it_imu != (v_imu.end() - 1); it_imu++)
  {
    ...
    vect3 acc_g_avr= cur_state.rot*(acc_avr * 10.0 - cur_state.ba); 
     for(int i = 0; i < 3; i++){
